    ...
    ui(new Ui::server)
{
    ...
    //开启监听
    this->serve->listen(QHostAddress::Any,8000);
    connect(this->serve,SIGNAL(newConnection()),this,SLOT(acceptConnection()));
...
{
    //返回一个socket连接
    this->socket = this->serve->nextPendingConnection();
    connect(socket,SIGNAL(readyRead()),this,SLOT(receiveData()));
    ui->browser->append("connect successful");
    tcpClient.append(socket);
    ui->cbxConnection->addItem(tr("%1:%2").arg(socket->peerAddress().toString().split("::ffff:")[1])\
                                          .arg(socket->peerPort()));
    this->socket->write("connect successfully");
}
...
void server::on_toolButton_clicked()
{
    ...
    ui->browser->append(filename_test);
}


void server::getIp()
{
    ui->browser->setText(QNetworkInterface().allAddresses().at(1).toString());
